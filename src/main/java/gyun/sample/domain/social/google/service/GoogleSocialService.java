...
public class GoogleSocialService implements SocialLoginService {
    ...
    private Member registerOrLoginMember(GoogleUserInfoResponse userInfo, String accessToken) {
        ...
        if (activeMember.isPresent()) {
            ...
            // 로그인 로그
            eventPublisher.publishEvent(MemberActivityEvent.of(member.getLoginId(), member.getId(), LogType.LOGIN, "GOOGLE 로그인", UtilService.getClientIp(httpServletRequest)));
            return member;
        }
        ...
        // 가입 로그
        eventPublisher.publishEvent(MemberActivityEvent.of(savedMember.getLoginId(), savedMember.getId(), LogType.JOIN, "GOOGLE 회원가입", UtilService.getClientIp(httpServletRequest)));
        return savedMember;
    }
}